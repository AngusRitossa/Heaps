// Stoer-Wagner's algorithm implemented with a strict fibonacci heap: O(ve + v^2 log v)
#include <cstdio>
#include <vector>
#include <utility>
#include <chrono>
...
FixlistNode fixlistallocarray[MAXN];
...
int main()
{
	...
	for (int c = 1; c <= v-1; c++) // Run this v-1 times
	{
		...
		for (int i = 0; i < v; i++)
		{
			...
			{
				nodes[i]->val.first = nodes[i]->inheap->loss = 0;
				nodes[i]->inheap->child = nodes[i]->inheap->left = nodes[i]->inheap->right = nodes[i]->inheap->parent = NULL;
				nodes[i]->inheap->active = passive;
				nodes[i]->inheap->rank = pq.ranklist;
				nodes[i]->inheap->qleft = nodes[i]->inheap->qright = NULL;
				pq.push(nodes[i]->inheap);
			}
		}
		int last;
		while (pq.size() > 1)
		{
			int a = pq.root->val->val.second; // A is the next node in the legal ordering
			last = a;
			inlegal[a] = c;
			pq.pop();
			for (pedge e = adj[a]; e; e = e->adj) 
			{
				...
				if (inlegal[b] != c)
				{
					pq.decreasekey(nodes[b]->inheap, nodes[b]->val.first - e->weight);
				}
			}
		}
